...
app.models.Post.Interactions = Backbone.Model.extend({
  ...
  comment : function (text) {
    ...
    this.comments.make(text).fail(function () {
      alert(Lygneo.I18n.t("failed_to_post_message"));
    }).done(function() {
  ...
  reshare : function(){
    var interactions = this
      ...
      , flash = new Lygneo.Widgets.FlashMessages;

    reshare.save({}, {
      success : function(resp){
        flash.render({
          success: true,
          notice: Lygneo.I18n.t("reshares.successful")
        });
      },
      error: function(resp){
        flash.render({
          success: false,
          notice: Lygneo.I18n.t("reshares.duplicate")
        });
      }
    }).done(function(){
  ...
  userCanReshare : function(){
    var isReshare = this.post.get("post_type") == "Reshare"
      ...
      , userIsNotAuthor = this.post.get("author").lygneo_id != app.currentUser.get("lygneo_id")
      , userIsNotRootAuthor = rootExists && (isReshare ? this.post.get("root").author.lygneo_id != app.currentUser.get("lygneo_id") : true)
