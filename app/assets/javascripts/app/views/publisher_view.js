...
//= require ./publisher/groups_selector
...
app.views.Publisher = Backbone.View.extend(_.extend(
  ...
  events : {
    ...
    "click .dropdown .dropdown_list li": "toggleGroup",
  ...
  createStatusMessage : function(evt) {
    ...
    statusMessage.save({
      ...
      "group_ids" : serializedForm["group_ids[]"],
  ...
  createPostPreview : function(evt) {
    ...
    while(user=regexp.exec(serializedForm["status_message[text]"])){
      ...
      var mentioned_user = Mentions.followers.filter(function(item) { return item.handle == user[2];})[0];
      if(mentioned_user){
    ...
    var previewMessage = {
      "id" : 0,
      "text" : serializedForm["status_message[text]"],
      "public" : serializedForm["group_ids[]"]=="public",
      "created_at" : date,
      "interacted_at" : date,
  ...
  open : function() {
    ...
    // fetch followers for mentioning
    Mentions.fetchFollowers();
    return this;
  },
