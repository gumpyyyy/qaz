app.views.TagFollowingAction = app.views.Base.extend({
  ...
  followString : function() {
    if(this.tag_is_followed()) {
      return Lygneo.I18n.t("stream.tags.following", {"tag" : this.model.attributes.name});
    } else {
      return Lygneo.I18n.t("stream.tags.follow", {"tag" : this.model.attributes.name});
    }
  },
  ...
  mouseIn : function(){
    this.$("input").removeClass("in_groups");
    this.$("input").val( Lygneo.I18n.t('stream.tags.stop_following', {tag: this.model.attributes.name} ) );
  },

  mouseOut : function() {
    this.$("input").addClass("in_groups");
    this.$("input").val( Lygneo.I18n.t('stream.tags.following', {"tag" : this.model.attributes.name} ) );
  },
