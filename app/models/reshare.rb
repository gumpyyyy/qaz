...
class Reshare < Post
  ...
  def root_lygneo_id
    self.root.author.lygneo_handle
  end
  ...
  def receive(recipient, sender)
    ...
    if local_reshare && local_reshare.root.author_id == recipient.person.id
      return unless recipient.has_follower_for?(sender)
    end
  ...
  def after_parse
    root_author = Webfinger.new(@root_lygneo_id).fetch
    ...
    if fetched_post
      ...
      if root_author.lygneo_handle != fetched_post.lygneo_handle
        raise "Lygneo ID (#{fetched_post.lygneo_handle}) in the root does not match the Lygneo ID (#{root_author.lygneo_handle}) specified in the reshare!"
      end
  ...
  def self.fetch_post author, guid
    ...
    Lygneo::Parser.from_xml(response.body)
  end
