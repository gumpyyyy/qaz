...
class RelayableRetraction < SignedRetraction
  ...
  def receive(recipient, sender)
    if self.target.nil?
      ...
      Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{recipient.lygneo_handle} sender=#{self.parent.author.lygneo_handle} payload_type=#{self.class} parent_id=#{self.parent.id}")
      return
    end
    self
  end
