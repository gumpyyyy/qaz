...
class Person < ActiveRecord::Base
  ...
  before_validation :downcase_lygneo_handle

  def downcase_lygneo_handle
    lygneo_handle.downcase! unless lygneo_handle.blank?
  end

  has_many :followers, :dependent => :destroy # Other people's followers for this person
  ...
  validates :lygneo_handle, :uniqueness => true

  scope :searchable, joins(:profile).where(:profiles => {:searchable => true})
  scope :remote, where('people.owner_id IS NULL')
  scope :local, where('people.owner_id IS NOT NULL')
  scope :for_json, select('DISTINCT people.id, people.guid, people.lygneo_handle').includes(:profile)

  # @note user is passed in here defensively
  scope :all_from_groups, lambda { |group_ids, user|
    joins(:followers => :group_pledges).
         where(:followers => {:user_id => user.id}).
         where(:group_pledges => {:group_id => group_ids})
  }

  scope :unique_from_groups, lambda{ |group_ids, user|
    all_from_groups(group_ids, user).select('DISTINCT people.*')
  }
  ...
  def self.search_query_string(query)
    ...
    where_clause = <<-SQL
      ...
      people.lygneo_handle #{like_operator} ?
    SQL
  ...
  def self.search(query, user)
    ...
    Person.searchable.where(sql, *tokens).joins(
      "LEFT OUTER JOIN followers ON followers.user_id = #{user.id} AND followers.person_id = people.id"
    ).includes(:profile
    ).order(search_order)
  end
  ...
  def self.search_order
    @search_order ||= Proc.new {
      ...
      ["followers.user_id #{order}", "profiles.last_name ASC", "profiles.first_name ASC"]
    }.call
  end
  ...
  def name(opts = {})
    ...
    @name ||= Person.name_from_attrs(self.profile.first_name, self.profile.last_name, self.lygneo_handle)
  end

  def self.name_from_attrs(first_name, last_name, lygneo_handle)
    first_name.blank? && last_name.blank? ? lygneo_handle : "#{first_name.to_s.strip} #{last_name.to_s.strip}".strip
  end

  def first_name
    @first_name ||= if profile.nil? || profile.first_name.nil? || profile.first_name.blank?
                self.lygneo_handle.split('@').first
              else
  ...
  def public_url
    if self.owner
      ...
      username = self.lygneo_handle.split("@")[0]
    end
    "#{url}public/#{username}"
  end
  ...
  def self.by_account_identifier(identifier)
    ...
    self.where(:lygneo_handle => identifier).first
  end
  ...
  def self.create_from_webfinger(profile, hcard)
    return nil if profile.nil? || !profile.valid_lygneo_profile?
    new_person = Person.new
    new_person.serialized_public_key = profile.public_key
    new_person.guid = profile.guid
    new_person.lygneo_handle = profile.account
    new_person.url = profile.seed_location

    #hcard_profile = HCard.find profile.hcard.first[:href]
    Rails.logger.info("event=webfinger_marshal valid=#{new_person.valid?} target=#{new_person.lygneo_handle}")
  ...
  def shares_with(user)
    user.followers.receiving.where(:person_id => self.id).first if user
  end
  ...
  def fix_profile
    Webfinger.new(self.lygneo_handle).fetch
    self.reload
  end
end
