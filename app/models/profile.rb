...
class Profile < ActiveRecord::Base
  ...
  def subscribers(user)
    Person.joins(:followers).where(:followers => {:user_id => user.id})
  end
  ...
  def lygneo_handle
    #get the parent lygneo handle, unless we want to access a profile without a person
    (self.person) ? self.person.lygneo_handle : self[:lygneo_handle]
  end
