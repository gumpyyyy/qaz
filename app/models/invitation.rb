...
class Invitation < ActiveRecord::Base
  ...
  attr_accessible :sender, :recipient, :group, :language, :service, :identifier, :admin, :message
 ...
 # before_create :share_with_exsisting_user, :if => :recipient_id?
  ...
  validates_presence_of :sender, :group, :unless => :admin?
  validate :ensure_not_inviting_self, :on => :create, :unless => :admin?
  validate :sender_owns_group?, :unless => :admin?
  ...
  def self.batch_invite(emails, opts)
    ...
    users_on_pod.each{|u| opts[:sender].share_with(u.person, opts[:group])}
  ...
  # @note Validation
  def sender_owns_group?
    if self.sender_id != self.group.user_id
      errors[:base] << 'You do not own that group.'
    end
  end
  ...
  def recipient_not_on_pod?
    ...
    if self.recipient.username?
      errors[:recipient] << "The user '#{self.identifier}' (#{self.recipient.lygneo_handle}) is already on this pod, so we sent them a share request"
    end
  end
