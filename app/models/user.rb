...
class User < ActiveRecord::Base
  ...
  delegate :guid, :public_key, :posts, :photos, :owns?, :image_url,
           :lygneo_handle, :name, :public_url, :profile, :url,
           :first_name, :last_name, :gender, :participations, to: :person
  delegate :id, :guid, to: :person, prefix: true

  has_many :invitations_from_me, :class_name => 'Invitation', :foreign_key => :sender_id
  has_many :invitations_to_me, :class_name => 'Invitation', :foreign_key => :recipient_id
  has_many :groups, :order => 'order_id ASC'

  belongs_to  :auto_follow_back_group, :class_name => 'Group'
  belongs_to :invited_by, :class_name => 'User'

  has_many :group_pledges, :through => :groups
  ...
  def self.all_sharing_with_person(person)
    User.joins(:followers).where(:followers => {:person_id => person.id})
  end
  ...
  ######### Groups ######################
  def add_follower_to_group(follower, group)
    return true if GroupPledge.exists?(:follower_id => follower.id, :group_id => group.id)
    follower.group_pledges.create!(:group => group)
  end

  ######## Posting ########
  def build_post(class_name, opts={})
    opts[:author] = self.person
    opts[:lygneo_handle] = opts[:author].lygneo_handle

    model_class = class_name.to_s.camelize.constantize
    model_class.lygneo_initialize(opts)
  end
  ...
  def notify_if_mentioned(post)
    return unless self.follower_for(post.author) && post.respond_to?(:mentions?)

    post.notify_person(self.person) if post.mentions? self.person
  end

  def add_to_streams(post, groups_to_insert)
    inserted_group_ids = groups_to_insert.map{|x| x.id}

    groups_to_insert.each do |group|
      group << post
    end
  end

  def groups_from_ids(group_ids)
  ...
  def salmon(post)
    Salmon::EncryptedSlap.create_by_user_and_activity(self, post.to_lygneo_xml)
  end
  ...
  def set_person(person)
    ...
    person.lygneo_handle = "#{self.username}#{User.lygneo_id_host}"
    self.person = person
  end

  def self.lygneo_id_host
    "@#{AppConfig.bare_pod_uri}"
  end

  def seed_groups
    self.groups.create(:name => I18n.t('groups.seed.family'))
    self.groups.create(:name => I18n.t('groups.seed.friends'))
    self.groups.create(:name => I18n.t('groups.seed.work'))
    aq = self.groups.create(:name => I18n.t('groups.seed.acquaintances'))
  ...
  def reorder_groups(group_order)
    i = 0
    group_order.each do |id|
      self.groups.find(id).update_attributes({ :order_id => i })
      i += 1
    end
  end
  ...
  def no_person_with_same_username
    lygneo_id = "#{self.username}#{User.lygneo_id_host}"
    if self.username_changed? && Person.exists?(:lygneo_handle => lygneo_id)
  ...
  def clear_account!
    ...
    [:getting_started,
     :disable_mail,
     :show_popular_in_stream].each do |field|
      self[field] = false
    end
  ...
  def clearable_fields
    self.attributes.keys - ["id", "username", "encrypted_password",
                            ...
                            "disable_mail", "show_popular_in_stream",
                            "email"]
  end
end
