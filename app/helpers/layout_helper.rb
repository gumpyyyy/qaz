...
module LayoutHelper
  ...
  def load_javascript_locales
    content_tag(:script) do
      <<-JS.html_safe
        Lygneo.I18n.loadLocale(#{get_javascript_strings_for(I18n.locale).to_json}, "#{I18n.locale}");
        Lygneo.Page = "#{params[:controller].camelcase}#{params[:action].camelcase}";
      JS
    end
  end
  ...
  def include_base_css_framework(use_bootstrap=false)
    if use_bootstrap || @group == :getting_started
