...
alice = FactoryGirl.create(:user_with_group, :username => "alice", :password => 'evankorth')
bob   = FactoryGirl.create(:user_with_group, :username => "bob", :password => 'evankorth')
eve   = FactoryGirl.create(:user_with_group, :username => "eve", :password => 'evankorth')
...
connect_users(bob, bob.groups.first, alice, alice.groups.first)
connect_users(bob, bob.groups.first, eve, eve.groups.first)
puts "done!"
...
(1..25).each do |n|
  [alice, bob, eve].each do |u|
    print '.'
    if(n%3==1)
      post = u.post :status_message, :text => "#{u.username} - #{n} - #seeded", :to => u.groups.first.id
    elsif(n%3==2)
      ...
      u.add_to_streams(post, u.groups)
    end
