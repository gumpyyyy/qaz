class MovePhotosToTheirOwnTable < ActiveRecord::Migration
  def self.up
    ...
    if AppConfig.postgres?
      ...
      execute "UPDATE group_visibilities SET shareable_type='Photo' FROM photos WHERE shareable_id=photos.id"
      execute "UPDATE share_visibilities SET shareable_type='Photo' FROM photos WHERE shareable_id=photos.id"
    else
      execute <<SQL
INSERT INTO photos
SELECT NULL as id, id AS tmp_old_id, author_id, public, lygneo_handle, guid, pending, text, remote_photo_path, remote_photo_name, random_string, processed_image,
created_at, updated_at, unprocessed_image, status_message_guid, comments_count
FROM posts
WHERE type = 'Photo'
SQL

      execute "UPDATE group_visibilities AS av, photos SET av.shareable_type='Photo' WHERE av.shareable_id=photos.id"
  ...
  def self.down
    if AppConfig.postgres?
      execute %{
        INSERT INTO posts (
          id, author_id, public, lygneo_handle, guid, pending, type, text,
          ...
          tmp_old_id, author_id, public, lygneo_handle, guid, pending, 'Photo', text,
          remote_photo_path, remote_photo_name, random_string, processed_image,
      ...
      execute %{
        ...
        WHERE
          ...
          AND photos.id=group_visibilities.shareable_id
        }
      ...
      execute <<SQL
INSERT INTO posts
  SELECT tmp_old_id AS id, author_id, public, lygneo_handle, guid, pending, 'Photo' AS type, text, remote_photo_path, remote_photo_name, random_string,
...
SQL
      ...
      execute <<SQL
UPDATE group_visibilities, posts, photos
SET
group_visibilities.shareable_id=posts.id,
group_visibilities.shareable_type='Post'
WHERE
posts.id=photos.tmp_old_id AND
photos.id=group_visibilities.shareable_id
SQL
