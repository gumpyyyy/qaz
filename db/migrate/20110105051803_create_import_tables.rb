class CreateImportTables < ActiveRecord::Migration
  def self.up
    [:groups, :comments, :followers, :invitations, :notifications, :people, :posts, :profiles, :requests, :services, :users].each do |table|
      add_column(table, :mongo_id, :string)
      add_index(table, :mongo_id)
    end

    add_column(:groups, :user_mongo_id, :string)
    create_table :mongo_groups do |t|
      t.string :mongo_id
      t.string :name
      t.string :user_mongo_id
      t.timestamps
    end
    add_index :mongo_groups, :user_mongo_id

    create_table :mongo_group_pledges do |t|
      t.string :group_mongo_id
      t.string :follower_mongo_id
      t.timestamps
    end
    add_index :mongo_group_pledges, :group_mongo_id
    add_index :mongo_group_pledges, :follower_mongo_id
    ...
    create_table :mongo_followers do |t|
      t.string :mongo_id
      t.string :user_mongo_id
      t.string :person_mongo_id
    ...
    add_index :mongo_followers, [:user_mongo_id, :pending]
    add_index :mongo_followers, [:person_mongo_id, :pending]
    ...
    add_index :mongo_people, :lygneo_handle, :unique => true
    ...
    add_index :mongo_post_visibilities, :group_mongo_id
  ...
  def self.down
    ...
    execute 'DROP TABLE mongo_followers'
    ...
    execute 'DROP TABLE mongo_group_pledges'
    execute 'DROP TABLE mongo_groups'
  end
end
