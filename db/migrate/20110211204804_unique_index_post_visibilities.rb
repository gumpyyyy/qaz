class UniqueIndexPostVisibilities < ActiveRecord::Migration
  def self.up
    ...
    unless visibility_ids.empty?
      sql = <<-SQL
      SELECT `post_visibilities`.post_id, `post_visibilities`.group_id FROM `post_visibilities`
        GROUP BY post_id, group_id
          HAVING COUNT(*)>1;
      SQL
    ...
    add_index :post_visibilities, [:group_id, :post_id], :unique => true
  end
