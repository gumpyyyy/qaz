...
#   the COPYRIGHT file.
module Lygneo; module Fetcher; class Public
  ...
  # with the given lygneo_id
  def fetch! lygneo_id
    @person = Person.by_account_identifier lygneo_id
  ...
  private
    ...
    def retrieve_and_process_posts
      begin
        ...
        FEDERATION_LOGGER.error "unable to retrieve public posts for #{@person.lygneo_handle}"
        raise e
      end
      ...
      begin
        ...
        FEDERATION_LOGGER.error "unable to process public posts for #{@person.lygneo_handle}"
        raise e
      end
    end
    ...
    def retrieve_posts
      ...
      FEDERATION_LOGGER.info "fetching public posts for #{@person.lygneo_handle}"

      resp = Faraday.get("#{@person.url}people/#{@person.guid}") do |req|
        req.headers[:accept] = 'application/json'
        req.headers[:user_agent] = 'lygneo-fetcher'
      end
    ...
    def process_posts
      @data.each do |post|
        ...
        entry = StatusMessage.lygneo_initialize(
          :author => @person,
          :public => true
        )
