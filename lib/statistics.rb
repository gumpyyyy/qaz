...
SQL
  ...
  def followers_sharing_with_count_sql
    <<SQL
      SELECT users.id AS id, count(followers.id) AS count
        FROM users
          JOIN followers on followers.user_id = users.id
          JOIN group_pledges on group_pledges.follower_id = followers.id
          #{self.where_clause_sql}
          GROUP BY users.id
SQL
  end
...
SQL
  ...
  def generate_correlations
    ...
    [:posts_count, :comments_count, :invites_sent_count, #:tags_followed_count,
     :mentions_count, :followers_sharing_with_count].each do |metric|
