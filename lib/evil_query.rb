module EvilQuery
  ...
  class MultiStream < Base
    ...
    def make_relation!
      post_ids = groups_post_ids! + ids!(followed_tags_posts!) + ids!(mentioned_posts)
      post_ids += ids!(popular_posts!) if @include_spotlight
      Post.where(:id => post_ids)
    end

    def groups_post_ids!
      @user.visible_shareable_ids(Post, :limit => 15, :order => "#{@order} DESC", :max_time => @max_time, :all_groups? => true, :by_members_of => @user.group_ids)
    end
    ...
    def popular_posts!
      Post.all_public.where(:author_id => fetch_ids!(Person.popular, 'people.id'))
    end
  ...
  class VisibleShareableById < Base
    ...
    def post!
      ...
      querent_is_follower.first || querent_is_author.first || public_post.first
    end

    protected

    def querent_is_follower
      @class.where(@key => @id).joins(:followers).where(:followers => {:user_id => @querent.id}).where(@conditions).select(@class.table_name+".*")
    end
  ...
  class ShareablesFromPerson < Base
    ...
    def make_relation!
      ...
      shareable_ids = follower.present? ? fetch_ids!(persons_private_visibilities, "share_visibilities.shareable_id") : []
    ...
    def follower
      @follower ||= @querent.follower_for(@person)
    end
    ...
    def persons_private_visibilities
      follower.share_visibilities.where(:hidden => false, :shareable_type => @class.to_s)
    end
