    ...
    doc.css('Link').each do |l|
      ...
      case rel
        when "http://microformats.org/profile/hcard"
          @hcard = href
        when "http://joinlygneo.com/guid"
          @guid = href
        when "http://joinlygneo.com/seed_location"
          @seed_location = href
      end
    end

    begin
      pubkey = text_of_attribute( doc.at('Link[rel=lygneo-public-key]'), 'href')
