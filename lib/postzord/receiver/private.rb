...
class Postzord::Receiver::Private < Postzord::Receiver
  ...
  def receive!
    begin 
      if @sender && self.salmon.verified_for_key?(@sender.public_key)
        ...
        FEDERATION_LOGGER.info("event=receive status=abort recipient=#{@user.lygneo_handle} sender=#{@salmon.author_id} reason='not_verified for key'")
        false
      end
    rescue => e
      #this sucks
  ...
  def parse_and_receive(xml)
    @object ||= Lygneo::Parser.from_xml(xml)
  ...
  def validate_object
    raise Lygneo::FollowerRequiredUnlessRequest if follower_required_unless_request
    raise Lygneo::RelayableObjectWithoutParent if relayable_without_parent?

    assign_sender_handle_if_request

    raise Lygneo::AuthorXMLAuthorMismatch if author_does_not_match_xml_author?
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      ...
      xml_author = @user.owns?(@object.parent) ? @object.lygneo_handle : @object.parent_author.lygneo_handle
      @author = Webfinger.new(@object.lygneo_handle).fetch if @object.author
    else
      xml_author = @object.lygneo_handle
    end
    xml_author
  end
  ...
  def relayable_without_parent?
    if @object.respond_to?(:relayable?) && @object.parent.nil?
      FEDERATION_LOGGER.error("event=receive status=abort reason='received a comment but no corresponding post' recipient=#{@user_person.lygneo_handle} sender=#{@sender.lygneo_handle} payload_type=#{@object.class})")
      return true
    end
  end

  def follower_required_unless_request
    unless @object.is_a?(Request) || @user.follower_for(@sender)
      FEDERATION_LOGGER.error("event=receive status=abort reason='sender not connected to recipient' recipient=#{@user_person.lygneo_handle} sender=#{@sender.lygneo_handle}")
      return true 
    end
  end
  ...
  def assign_sender_handle_if_request
    ...
    if @object.is_a?(Request)
      @object.sender_handle = @sender.lygneo_handle
    end
  end
end
