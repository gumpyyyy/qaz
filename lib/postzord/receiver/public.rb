...
class Postzord::Receiver::Public < Postzord::Receiver
  ...
  # @return [Object]
  def save_object
    @object = Lygneo::Parser::from_xml(@salmon.parsed_data)
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      #this is public, so it would only be owners sending us other people comments etc
       @object.parent_author.local? ? @object.lygneo_handle : @object.parent_lygneo_handle
    else
      @object.lygneo_handle
    end
  end
  ...
  def account_deletion_is_from_author
    ...
    return false if @object.lygneo_handle != @author.lygneo_handle
    return true
  end
