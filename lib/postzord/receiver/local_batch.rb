...
class Postzord::Receiver::LocalBatch < Postzord::Receiver
  ...
  def create_share_visibilities
    followers_ids = Follower.connection.select_values(Follower.where(:user_id => @recipient_user_ids, :person_id => @object.author_id).select("id").to_sql)
    ShareVisibility.batch_import(followers_ids, object)
  end
