...
describe ConversationsController do
  ...
  describe '#new' do
    ...
    it "assigns a json list of followers that are sharing with the person" do
      assigns(:followers_json).should include(alice.followers.where(:sharing => true).first.person.name)
      alice.followers << Follower.new(:person_id => eve.person.id, :user_id => alice.id, :sharing => false, :receiving => true)
      assigns(:followers_json).should_not include(alice.followers.where(:sharing => false).first.person.name)
    end

    it "assigns a follower if passed a follower id" do
      get :new, :follower_id => alice.followers.first.id
      assigns(:follower_ids).should == alice.followers.first.id
    end

    it "assigns a set of followers if passed an group id" do
      get :new, :group_id => alice.groups.first.id
      assigns(:follower_ids).should == alice.groups.first.followers.map(&:id).join(',')
    end
  ...
  describe '#index' do
    before do
      hash = {
        :author => alice.person,
        :participant_ids => [alice.followers.first.person.id, alice.person.id],
        :subject => 'not spam',
  ...
  describe '#create' do
    context 'with a valid conversation' do
      before do
        @hash = {
          ...
          :follower_ids => [alice.followers.first.id]
        }
      end
    ...
    context 'with empty follower' do
      before do
        @hash = {
          :conversation => {
            :subject => 'secret stuff',
      ...
      it 'should set response with success to false and message to fail due to no follower' do
        post :create, @hash
        assigns[:response][:success].should == false
        assigns[:response][:message].should == I18n.t('conversations.create.no_follower')
      end
    end

    context 'with nil follower' do
      before do
        @hash = {
          :conversation => {
            :subject => 'secret stuff',
