...
describe NotificationsController do
  ...
  describe '#index' do
    ...
    describe "special case for start sharing notifications" do
      it "should not provide a followers menu for standard notifications" do
        2.times { FactoryGirl.create(:notification, :recipient => alice, :target => @post) }
        get :index, "per_page" => 5

        Nokogiri(response.body).css('.group_pledge').should be_empty
      end
      it "should provide a followers menu for start sharing notifications" do
        2.times { FactoryGirl.create(:notification, :recipient => alice, :target => @post) }
        eve.share_with(alice.person, eve.groups.first)
        get :index, "per_page" => 5

        Nokogiri(response.body).css('.group_pledge').should_not be_empty
      end
