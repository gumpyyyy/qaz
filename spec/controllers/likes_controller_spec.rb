    ...
    context class_const.to_s do
      ...
      describe '#create' do
        ...
        context "on my own post" do
          it 'succeeds' do
            @target = alice.post :status_message, :text => "AWESOME", :to => @alices_group.id
        ...
        context "on a post from a follower" do
          before do
            @target = bob.post(:status_message, :text => "AWESOME", :to => @bobs_group.id)
        ...
        context "on a post from a stranger" do
          before do
            @target = eve.post :status_message, :text => "AWESOME", :to => eve.groups.first.id
      ...
      describe '#index' do
        before do
          @message = alice.post(:status_message, :text => "hey", :to => @alices_group.id)
      ...
      describe '#destroy' do
        before do
          @message = bob.post(:status_message, :text => "hey", :to => @alices_group.id)
