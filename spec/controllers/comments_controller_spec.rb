...
describe CommentsController do
  ...
  describe '#create' do
    ...
    context "on my own post" do
      before do
        ...
        @post = alice.post :status_message, :text => 'GIANTS', :to => group_to_post
      end
    ...
    context "on a post from a follower" do
      before do
        group_to_post = bob.groups.where(:name => "generic").first
        @post = bob.post :status_message, :text => 'GIANTS', :to => group_to_post
      end
    ...
    it 'posts no comment on a post from a stranger' do
      group_to_post = eve.groups.where(:name => "generic").first
      @post = eve.post :status_message, :text => 'GIANTS', :to => group_to_post
  ...
  describe '#destroy' do
    before do
      group_to_post = bob.groups.where(:name => "generic").first
      @message = bob.post(:status_message, :text => "hey", :to => group_to_post)
    end
  ...
  describe '#index' do
    before do
      group_to_post = bob.groups.where(:name => "generic").first
      @message = bob.post(:status_message, :text => "hey", :to => group_to_post.id)
    end
    ...
    it 'returns a 404 on a post that is not visible to the signed in user' do
      group_to_post = eve.groups.where(:name => "generic").first
      message = eve.post(:status_message, :text => "hey", :to => group_to_post.id)
