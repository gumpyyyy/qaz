...
describe PostsController do
  before do
    ...
    @message = alice.build_post :status_message, :text => "ohai", :to => group.id
    @message.save!

    alice.add_to_streams(@message, [group])
    alice.dispatch_post @message, :to => group.id
  end
  ...
  describe '#show' do
    ...
    context 'user not signed in' do
      context 'given a public post' do
        ...
        it 'responds with lygneo xml if format is xml' do
          get :show, :id => @status.guid, :format => :xml
          response.body.should == @status.to_lygneo_xml
        end
      end
  ...
  describe '#destroy' do
    ...
    it 'let a user delete his message' do
      message = alice.post(:status_message, :text => "hey", :to => alice.groups.first.id)
    ...
    it 'will not let you destroy posts visible to you' do
      message = bob.post(:status_message, :text => "hey", :to => bob.groups.first.id)
    ...
    it 'will not let you destory posts you do not own' do
      message = eve.post(:status_message, :text => "hey", :to => eve.groups.first.id)
      expect { delete :destroy, :format => :js, :id => message.id }.to raise_error(ActiveRecord::RecordNotFound)
      StatusMessage.exists?(message.id).should be_true
    end
  end
