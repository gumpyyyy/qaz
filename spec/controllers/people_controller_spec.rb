...
describe PeopleController do
  ...
  describe '#index (search)' do
    ...
    describe 'via html' do
      context 'query is a lygneo ID' do
        before do
          @unsearchable_eugene = FactoryGirl.create(:person, :lygneo_handle => "eugene@example.org",
      ...
      context 'query is not a tag or a lygneo ID' do
  ...
  describe "#show performance", :performance => true do
    before do
      ...
      8.times do |n|
        ...
        @users << user
        group = user.groups.create(:name => 'people')
        connect_users(@user, @user.groups.first, user, group)

        @posts << @user.post(:status_message, :text => "hello#{n}", :to => group.id)
      end
      @posts.each do |post|
        @users.each do |user|
  ...
  describe '#show' do
    ...
    context "when the person is the current user" do
      ...
      it "assigns all the user's posts" do
        ...
        @user.post(:status_message, :text => "to one group", :to => @group.id)
        @user.post(:status_message, :text => "to all groups", :to => 'all')
      ...
      it "renders the comments on the user's posts" do
        message = @user.post :status_message, :text => 'test more', :to => @group.id
    ...
    context "with no user signed in" do
      ...
      context 'with posts' do
        before do
          @public_posts = []
          @public_posts << bob.post(:status_message, :text => "first public ", :to => bob.groups[0].id, :public => true)
          bob.post(:status_message, :text => "to an group @user is not in", :to => bob.groups[1].id)
          bob.post(:status_message, :text => "to all groups", :to => 'all')
        ...
        it "posts include reshares" do
          reshare = @user.post(:reshare, :public => true, :root_guid => FactoryGirl.create(:status_message, :public => true).guid, :to => alice.groups)
          get :show, :id => @user.person.to_param
    ...
    context "when the person is a follower of the current user" do
      before do
        @person = bob.person
      end
      ...
      it "assigns only the posts the current user can see" do
        ...
        posts_user_can_see = []
        group_user_is_in = bob.groups.where(:name => "generic").first
        group_user_is_not_in = bob.groups.where(:name => "empty").first
        posts_user_can_see << bob.post(:status_message, :text => "to an group @user is in", :to => group_user_is_in.id)
        bob.post(:status_message, :text => "to an group @user is not in", :to => group_user_is_not_in.id)
        posts_user_can_see << bob.post(:status_message, :text => "to all groups", :to => 'all')
    ...
    context "when the person is not a follower of the current user" do
      before do
        @person = eve.person
      end
      ...
      it "assigns only public posts" do
        ...
        eve.post(:status_message, :text => "to an group @user is not in", :to => eve.groups.first.id)
        eve.post(:status_message, :text => "to all groups", :to => 'all')
  ...
  describe '#hovercard' do
    ...
    it 'returns json with profile stuff' do
      ...
      JSON.parse( response.body )['handle'].should == @hover_test.lygneo_handle
    end
  end
  ...
  describe '#refresh_search ' do
    ...
    describe 'via json' do
      ...
      it 'returns with a found name' do
        get :refresh_search, :q => @korth.lygneo_handle
  ...
  describe '#followers' do
    it 'assigns the followers of a person' do
      follower = alice.follower_for(bob.person)
      followers = follower.followers
      get :followers, :person_id => bob.person.to_param
      assigns(:followers_of_follower).should =~ followers
      response.should be_success
    end

    it 'shows an error when invalid person id' do
      get :followers, :person_id => 'foo'
      flash[:error].should be_present
      response.should redirect_to people_path
    end
  end

  describe '#lygneo_id?' do
    it 'returns true for pods on urls' do
      @controller.send(:lygneo_id?, "ilya_123@pod.geraspora.de").should be_true
    end

    it 'returns true for pods on urls with port' do
      @controller.send(:lygneo_id?, "ilya_123@pod.geraspora.de:12314").should be_true
    end

    it 'returns true for pods on localhost' do
      @controller.send(:lygneo_id?, "ilya_123@localhost").should be_true
    end

    it 'returns true for pods on localhost and port' do
      @controller.send(:lygneo_id?, "ilya_123@localhost:1234").should be_true
    end

    it 'returns true for pods on ip' do
      @controller.send(:lygneo_id?, "ilya_123@1.1.1.1").should be_true
    end

    it 'returns true for pods on ip and port' do
      @controller.send(:lygneo_id?, "ilya_123@1.2.3.4:1234").should be_true
    end

    it 'returns false for pods on with invalid url characters' do
      @controller.send(:lygneo_id?, "ilya_123@join_lygneo.com").should be_false
    end

    it 'returns false for invalid usernames' do
      @controller.send(:lygneo_id?, "ilya_2%3@joinlygneo.com").should be_false
    end
  end
end
