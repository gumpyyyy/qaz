...
describe("app.views.Publisher", function() {
  ...
  context("#toggleService", function(){
    beforeEach( function(){
      spec.loadFixture('groups_index_services');
      this.view = new app.views.Publisher();
    });
  ...
  context("group selection", function(){
    beforeEach( function(){
      spec.loadFixture('status_message_new');

      this.radio_els = $('#publisher .dropdown li.radio');
      this.check_els = $('#publisher .dropdown li.group_selector');

      this.view = new app.views.Publisher();
      this.view.open();
    });

    it("initializes with 'all_groups'", function(){
    ...
    describe("#_updateSelectedGroupIds", function(){
      beforeEach(function(){
        this.li = $('<li data-group_id="42" />');
        this.view.$('.dropdown_list').append(this.li);
      });

      it("gets called when groups are selected", function(){
        spyOn(this.view, "_updateSelectedGroupIds");
        this.check_els.last().trigger('click');
        expect(this.view._updateSelectedGroupIds).toHaveBeenCalled();
      });

      it("removes a previous selection and inserts the current one", function() {
        var selected = this.view.$('input[name="group_ids[]"]');
        expect(selected.length).toBe(1);
        expect(selected.first().val()).toBe('all_groups');

        this.li.trigger('click');

        selected = this.view.$('input[name="group_ids[]"]');
        expect(selected.length).toBe(1);
        expect(selected.first().val()).toBe('42');
      });

      it("toggles the same item", function() {
        expect(this.view.$('input[name="group_ids[]"]').length).toBe(1);

        this.li.trigger('click');
        expect(this.view.$('input[name="group_ids[]"]').length).toBe(1);

        this.li.trigger('click');
        expect(this.view.$('input[name="group_ids[]"]').length).toBe(0);
      });

      it("keeps other fields with different values", function() {
        var li2 = $("<li data-group_id=99></li>");
        ...
        li2.trigger('click');
        expect(this.view.$('input[name="group_ids[]"]').length).toBe(2);
      });
    });

    describe("#_addHiddenGroupInput", function(){
      it("gets called when groups are selected", function(){
        spyOn(this.view, "_addHiddenGroupInput");
        this.check_els.last().trigger('click');
        expect(this.view._addHiddenGroupInput).toHaveBeenCalled();
      });

      it("adds a hidden input to the form", function(){
        var id = 42;

        this.view._addHiddenGroupInput(id);
        var input = this.view.$('input[name="group_ids[]"][value="'+id+'"]');
