...
describe Stream::Base do
  ...
  describe '#followers_link' do
    it 'should default to your followers page' do
      @stream.followers_link.should =~ /followers/
    end
  end
  ...
  describe '#stream_posts' do
    ...
    context "when alice has liked some posts" do
      before do
        bob.post(:status_message, :text => "sup", :to => bob.groups.first.id)
  ...
  describe '.can_comment?' do
    ...
    it 'allows me to comment on my local followers post' do
    ...
    it 'allows me to comment on a remote followers post' do
      Follower.create!(:user => @stream.user, :person => @person)
      post = FactoryGirl.create(:status_message, :author => @person)
      @stream.can_comment?(post).should be_true
    end

    it 'returns false if person is remote and not a follower' do
      post = FactoryGirl.create(:status_message, :author => @person)
