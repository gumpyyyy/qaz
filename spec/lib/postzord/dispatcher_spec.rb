...
describe Postzord::Dispatcher do
  ...
  describe '.initialize' do
    it 'sets @sender, @object, @xml' do
      ...
      zord.xml.should == @sm.to_lygneo_xml
    end
    ...
    it 'raises and gives you a helpful message if the object can not federate' do
      ...
      }.to raise_error /Lygneo::Federated::Base/
    end
  end
  ...
  context 'instance methods' do
    ...
    context "comments" do
      ...
      context "local luke's post is commented on by" do
        before do
          @post = @local_luke.post(:status_message, :text => "hello", :to => @local_luke.groups.first)
        end
    ...
    describe '#deliver_to_services' do
      before do
        alice.groups.create(:name => "whatever")
      ...
      it 'does not push to hub for non-public posts' do
       ...
       mailman = Postzord::Dispatcher.build(alice, @sm, :url => "http://joinlygneo.com/p/123")
      ...
      it 'only pushes to specified services' do
       ...
       alice.services << @s2
       mailman = Postzord::Dispatcher.build(alice, FactoryGirl.create(:status_message), :url => "http://joinlygneo.com/p/123", :services => [@s1])
      ...
      it 'does not push to services if none are specified' do
       mailman = Postzord::Dispatcher.build(alice, FactoryGirl.create(:status_message), :url => "http://joinlygneo.com/p/123")
      ...
      it 'queues a job to delete if given retraction' do
        ...
        mailman = Postzord::Dispatcher.build(alice, retraction,  :url => "http://joinlygneo.com/p/123", :services => [@service])
