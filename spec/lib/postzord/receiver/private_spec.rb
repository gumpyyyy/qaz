...
describe Postzord::Receiver::Private do
  ...
  before do
    @alices_post = alice.build_post(:status_message, :text => "hey", :group_ids => [alice.groups.first.id])
  ...
  describe '#receive!' do
    ...
    context 'returns the sent object' do
      it 'returns the received object on success' do
        @zord.receive!
        @zord.instance_variable_get(:@object).should respond_to(:to_lygneo_xml)
      end
    end

    it 'parses the salmon object' do
      Lygneo::Parser.should_receive(:from_xml).with(@salmon.parsed_data).and_return(@alices_post)
      @zord.receive!
    end
  end
