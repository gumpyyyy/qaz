...
describe Lygneo::Fetcher::Public do
  before do
  ...
  describe "#retrieve_posts" do
    ...
    it "sets the operation status on the person" do
      @person.reload
      @person.fetch_status.should_not eql(Lygneo::Fetcher::Public::Status_Initial)
      @person.fetch_status.should eql(Lygneo::Fetcher::Public::Status_Fetched)
    end
  ...
  describe "#process_posts" do
    ...
    it 'sets the operation status on the person' do
      ...
      @person.reload
      @person.fetch_status.should_not eql(Lygneo::Fetcher::Public::Status_Initial)
      @person.fetch_status.should eql(Lygneo::Fetcher::Public::Status_Processed)
    end
  ...
  context "private methods" do
    let(:public_fetcher) { Lygneo::Fetcher::Public.new }
    ...
    describe '#qualifies_for_fetching?' do
      ...
      it 'returns false if the person is unfetchable' do
        public_fetcher.instance_eval {
          @person = FactoryGirl.create(:person, {:fetch_status => Lygneo::Fetcher::Public::Status_Unfetchable})
          qualifies_for_fetching?
        }.should be_false
      end
      ...
      it 'returns false and sets the person unfetchable for a local account' do
        ...
        user.person.fetch_status.should eql Lygneo::Fetcher::Public::Status_Unfetchable
      end

      it 'returns false if the person is processing already (or has been processed)' do
        person = FactoryGirl.create(:person)
        person.fetch_status = Lygneo::Fetcher::Public::Status_Fetched
        person.save
        public_fetcher.instance_eval {
          @person = person
          qualifies_for_fetching?
      ...
      it "returns true, if the user is remote and hasn't been fetched" do
        person = FactoryGirl.create(:person, {:lygneo_handle => 'neo@theone.net'})
        public_fetcher.instance_eval {
          @person = person
          qualifies_for_fetching?
        }.should be_true
    ...
    describe '#set_fetch_status' do
      it 'sets the current status of fetching on the person' do
        ...
        public_fetcher.instance_eval {
          @person = person
          set_fetch_status Lygneo::Fetcher::Public::Status_Unfetchable
        }
        @person.fetch_status.should eql Lygneo::Fetcher::Public::Status_Unfetchable

        public_fetcher.instance_eval {
          set_fetch_status Lygneo::Fetcher::Public::Status_Initial
        }
        @person.fetch_status.should eql Lygneo::Fetcher::Public::Status_Initial
      end
    end
