...
describe Salmon::Slap do
  before do
    @post = alice.post(:status_message, :text => "hi", :to => alice.groups.create(:name => "abcd").id)
    @created_salmon = Salmon::Slap.create_by_user_and_activity(alice, @post.to_lygneo_xml)
  end
  ...
  it 'works' do
    ...
    salmon.parsed_data.should == @post.to_lygneo_xml
  end
  ...
  describe "#process_header" do
    it 'sets the author id' do
      ...
      slap.author_id.should == alice.lygneo_handle
    end
  end
  ...
  describe '#author' do
    ...
    it 'should fail if no author is found' do
      parsed_salmon.author_id = 'tom@tom.joinlygneo.com'
      expect {
  ...
  context 'marshaling' do
    ...
    it 'should parse out the authors lygneo_handle' do
      parsed_salmon.author_id.should == alice.person.lygneo_handle
    end
    ...
    it 'contains the original data' do
      parsed_salmon.parsed_data.should == @post.to_lygneo_xml
    end
  end
  ...
  describe "#xml_for" do
    ...
    it "has lygneo as the root" do
      doc = Nokogiri::XML(@xml)
      doc.root.name.should == "lygneo"
    end
    ...
    context "header" do
      ...
      it "it has author_id node " do
        ...
        search.map(&:text).should == [alice.lygneo_handle]
      end
