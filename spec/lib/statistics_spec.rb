...
describe Statistics do
  ...
  describe '#invites_sent_count_sql' do
    it "pulls back an array of invite counts and ids" do
      Invitation.batch_invite(["a@a.com"], :sender => bob, :group => bob.groups.first, :service => 'email')
  ...
  describe '#followers_sharing_with_count_sql' do
    it "pulls back an array of mentions following counts and ids" do
      ...
      result_should_equal User.connection.select_all(@stats.followers_sharing_with_count_sql)
    end
  end
  ...
  ["posts_count", "comments_count", "invites_sent_count", "tags_followed_count",
    "mentions_count", "sign_in_count", "followers_sharing_with_count" ].each do |method|
