...
describe Notifier do
  ...
  describe ".private_message" do
    before do
      @user2 = bob
      @participant_ids = @user2.followers.map{|c| c.person.id} + [ @user2.person.id]
    ...
    it "FROM: contains the sender's name" do
      @mail["From"].to_s.should == "\"#{@cnv.author.name} (Lygneo*)\" <#{AppConfig.mail.sender_address}>"
    end
  ...
  context "comments" do
    ...
    describe ".comment_on_post" do
      ...
      it "FROM: contains the sender's name" do
        comment_mail["From"].to_s.should == "\"#{eve.name} (Lygneo*)\" <#{AppConfig.mail.sender_address}>"
      end
