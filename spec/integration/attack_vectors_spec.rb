...
def receive_public(post, opts)
  sender = opts.fetch(:from)
  salmon_xml = Salmon::Slap.create_by_user_and_activity(sender, post.to_lygneo_xml).xml_for(nil)
  post.destroy
...
def legit_post_from_user1_to_user2(user1, user2)
  original_message = user1.post(:status_message, :text => 'store this!', :to => user1.groups.find_by_name("generic").id)
...
describe "attack vectors" do
  ...
  let(:eves_group) { eve.groups.find_by_name("generic") }
  let(:alices_group) { alice.groups.find_by_name("generic") }

  context "testing side effects of validation phase" do

    describe 'Follower Required Unless Request' do
      #CUSTOM SETUP; cant use helpers here
      it 'does not save a post from a non-follower as a side effect' do
        salmon_xml = nil
        bad_post_guid = nil

        temporary_user do |bad_user|
          temporary_post(bad_user) do |post_from_non_follower|
            bad_post_guid = post_from_non_follower.guid
            salmon_xml = bad_user.salmon(post_from_non_follower).xml_for(bob.person)
          end
        end
        ...
        expect {
          expect {
            zord.perform!
          }.to raise_error Lygneo::FollowerRequiredUnlessRequest
      ...
      #CUSTOM SETUP; cant use helpers here
      it 'other users can not grant visiblity to another users posts by sending their friends post to themselves (even if they are followers)' do
        ...
        original_message = eve.post(:status_message, :text => 'store this!', :to => eves_group.id)
        original_message.lygneo_handle = bob.lygneo_handle

        alice.followers.create(:person => eve.person, :groups => [alice.groups.first])
    ...
    describe 'author does not match xml author' do
      it 'should not overwrite another persons profile profile' do
        ...
        expect {
          ...
          }.to raise_error Lygneo::AuthorXMLAuthorMismatch
  ...
  context 'malicious follower attack vector' do
    ...
    it "ignores retractions on a post not owned by the retraction's sender" do
      ...
      ret = bogus_retraction do |retraction| 
        ...
        retraction.lygneo_handle = alice.person.lygneo_handle
    ...
    it "silently disregards retractions for non-existent posts(that are from someone other than the post's author)" do
      bogus_retraction = temporary_post(eve) do |original_message|
                            bogus_retraction do |ret|
                              ...
                              ret.lygneo_handle = alice.person.lygneo_handle
    ...
    it 'should not receive retractions where the retractor and the salmon author do not match' do
      ...
      retraction = bogus_retraction do |ret|
        ret.post_guid = original_message.guid
        ret.lygneo_handle = eve.person.lygneo_handle
        ret.type = original_message.class.to_s
      end
    ...
    it 'it should not allow you to send retractions for other people' do
      ...
      expect{
        receive(retraction, :from => alice, :by => bob)
      }.to_not change{bob.reload.followers.count}
    end
    ...
    it 'it should not allow you to send retractions with xml and salmon handle mismatch' do
      ...
      }.to_not change(bob.followers, :count)
    end

    it 'does not let another user update other persons post' do
      original_message = eve.post(:photo, :user_file => uploaded_photo, :text => "store this!", :to => eves_group.id)
      receive(original_message, :from => eve, :by => bob)

      #is this testing two things?
      new_message = original_message.dup
      new_message.lygneo_handle = alice.lygneo_handle
