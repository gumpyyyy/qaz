...
describe 'a user receives a post' do
  ...
  it 'should be able to parse and store a status message from xml' do
    status_message = bob.post :status_message, :text => 'store this!', :to => @bobs_group.id

    xml = status_message.to_lygneo_xml
    bob.delete
  ...
  it 'should not create new groups on message receive' do
    num_groups = alice.groups.size

    2.times do |n|
      status_message = bob.post :status_message, :text => "store this #{n}!", :to => @bobs_group.id
    end

    alice.groups.size.should == num_groups
  end
  ...
  it "should show bob's post to alice" do
    inlined_jobs do |queue|
      ...
      sm.save!
      bob.groups.reload
      bob.add_to_streams(sm, [@bobs_group])
      queue.drain_all
      bob.dispatch_post(sm, :to => @bobs_group)
    end
  ...
  context 'with mentions, ' do
    it 'adds the notifications for the mentioned users regardless of the order they are received' do
      ...
      @sm = bob.build_post(:status_message, :text => "@{#{alice.name}; #{alice.lygneo_handle}} stuff @{#{eve.name}; #{eve.lygneo_handle}}")
      bob.add_to_streams(@sm, [bob.groups.first])
      @sm.save
    ...
    it 'notifies local users who are mentioned' do
      @remote_person = FactoryGirl.create(:person, :lygneo_handle => "foobar@foobar.com")
      Follower.create!(:user => alice, :person => @remote_person, :groups => [@alices_group])

      Notification.should_receive(:notify).with(alice, anything(), @remote_person)

      @sm = FactoryGirl.create(:status_message, :text => "hello @{#{alice.name}; #{alice.lygneo_handle}}", :lygneo_handle => @remote_person.lygneo_handle, :author => @remote_person)
      @sm.save
    ...
    it 'does not notify the mentioned user if the mentioned user is not friends with the post author' do
      ...
      @sm = eve.build_post(:status_message, :text => "should not notify @{#{alice.name}; #{alice.lygneo_handle}}")
      eve.add_to_streams(@sm, [eve.groups.first])
      @sm.save
  ...
  context 'update posts' do
    it 'does not update posts not marked as mutable' do
      status = alice.post :status_message, :text => "store this!", :to => @alices_group.id
      status.text = 'foo'
      xml = status.to_lygneo_xml
    ...
    it 'updates posts marked as mutable' do
      photo = alice.post(:photo, :user_file => uploaded_photo, :text => "Original", :to => @alices_group.id)
      photo.text = 'foo'
      xml = photo.to_lygneo_xml
      bob.reload
  ...
  describe 'profiles' do
   it 'federates tags' do
     ...
     raph.profile.lygneo_handle = "raph@remote.net"
     raph.profile.save!
     p = raph.profile
  ...
  describe 'post refs' do
    before do
      @status_message = bob.post(:status_message, :text => "hi", :to => @bobs_group.id)
      alice.reload
      @alices_group.reload
      @follower = alice.follower_for(bob.person)
    end

    it "adds a received post to the the follower" do
      alice.visible_shareables(Post).should include(@status_message)
      @follower.posts.should include(@status_message)
    end

    it 'removes posts upon forceful removal' do
      alice.remove_follower(@follower, :force => true)
      alice.reload
    ...
    context 'dependent delete' do
      it 'deletes share_visibilities on disconnected by' do
        ...
        alice.followers.create(:person => @person, :groups => [@alices_group])

        @post = FactoryGirl.create(:status_message, :author => @person)
        @post.share_visibilities.should be_empty
        receive_with_zord(alice, @person, @post.to_lygneo_xml)
        @follower = alice.follower_for(@person)
        @follower.share_visibilities.reset
        @follower.posts(true).should include(@post)
  ...
  describe 'comments' do
    ...
    context 'remote' do
      before do
        inlined_jobs do |queue|
          connect_users(alice, @alices_group, eve, @eves_group)
          @post = alice.post(:status_message, :text => "hello", :to => @alices_group.id)

          xml = @post.to_lygneo_xml
          ...
          # before relaying it out to the followers on the top-level post
          ...
          queue.drain_all
          @xml_with_whitespace = comment_with_whitespace.to_lygneo_xml
      ...
      it 'should correctly marshal a stranger for the downstream user' do
        ...
        m = mock()
        Webfinger.should_receive(:new).twice.with(eve.person.lygneo_handle).and_return(m)
  ...
  describe 'salmon' do
    let(:post){alice.post :status_message, :text => "hello", :to => @alices_group.id}
  ...
  context 'retractions' do
    ...
    it 'should accept signed retractions for public posts' do
      ...
      salmon = Postzord::Dispatcher::Public.salmon bob, retraction.to_lygneo_xml
