...
describe PeopleHelper do
 ...
 describe "#person_image_link" do
  ...
  describe '#person_link' do
    ...
    it 'uses lygneo handle if the person has no first or last name' do
      @person.profile.first_name = nil
      @person.profile.last_name = nil

      person_link(@person).should include @person.lygneo_handle
    end

    it 'uses lygneo handle if first name and first name are rails#blank?' do
  ...
  describe '#local_or_remote_person_path' do
    ...
    it "links by id if there is a period in the user's username" do
      ...
      person = @user.person
      person.lygneo_handle = "#{@user.username}@#{AppConfig.pod_uri.authority}"
      person.save!
  ...
  describe '#sharing_message' do
    before do
      @follower = FactoryGirl.create(:follower, :person => @person)
    end

    context 'when the follower is sharing' do
      it 'shows the sharing message' do
        message = I18n.t('people.helper.is_sharing', :name => @person.name)
        @follower.stub(:sharing?).and_return(true)
        sharing_message(@person, @follower).should include(message)
      end
    end

    context 'when the follower is not sharing' do
      it 'does not show the sharing message' do
        @follower.stub(:sharing?).and_return(false)
        sharing_message(@person, @follower).should be_blank
      end
    end
  end
end
