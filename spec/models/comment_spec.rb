...
describe Comment do
  before do
    @alices_group = alice.groups.first
    @status = bob.post(:status_message, :text => "hello", :to => bob.groups.first.id)
  end
  ...
  describe 'User#comment' do
    ...
    it "should be able to comment on a follower's status" do
  ...
  describe 'xml' do
    before do
      ...
      @commenter_group = @commenter.groups.create(:name => "bruisers")
      connect_users(alice, @alices_group, @commenter, @commenter_group)
    ...
    it 'serializes the sender handle' do
      @xml.include?(@commenter.lygneo_handle).should be_true
    end
  ...
  describe 'it is relayable' do
    before do
      ...
      @local_parent = @local_luke.post :status_message, :text => "hi", :to => @local_luke.groups.first
