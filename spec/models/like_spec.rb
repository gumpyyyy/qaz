...
describe Like do
  ...
  describe 'xml' do
    before do
      ...
      @liker_group = @liker.groups.create(:name => "dummies")
      connect_users(alice, alices_group, @liker, @liker_group)
      @post = alice.post(:status_message, :text => "huhu", :to => alices_group.id)
      @like = @liker.like!(@post)
      @xml = @like.to_xml.to_s
    end
    it 'serializes the sender handle' do
      @xml.include?(@liker.lygneo_handle).should be_true
    end
  ...
  describe 'it is relayable' do
    before do
      ...
      @local_parent = @local_luke.post :status_message, :text => "foobar", :to => @local_luke.groups.first
