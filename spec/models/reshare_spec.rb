...
describe Reshare do
  ...
  describe "#receive" do
    ...
    it 'does not error if the root author has a follower for the resharer' do
      bob.share_with @reshare.author, bob.groups.first
      proc {
  ...
  describe "XML" do
    ...
    context 'serialization' do
      it 'serializes root_lygneo_id' do
        @xml.should include("root_lygneo_id")
        @xml.should include(@reshare.author.lygneo_handle)
      end
    ...
    context 'marshalling' do
      context 'local' do
        ...
        it 'fetches the root author from root_lygneo_id' do
      ...
      context 'remote' do
        ...
        it 'fetches the root author from root_lygneo_id' do
          ...
          @response.stub(:body).and_return(@root_object.to_lygneo_xml)
        ...
        context 'saving the post' do
          ...
          it 'fetches the root post from root_guid' do
            ...
            [:text, :guid, :lygneo_handle, :type, :public].each do |attr|
          ...
          it 'verifies that the author of the post received is the same as the author in the reshare xml' do
            ...
            lambda{
              Reshare.from_xml(@xml)
            }.should raise_error /^Lygneo ID \(.+\) in the root does not match the Lygneo ID \(.+\) specified in the reshare!$/
          end
        end
      end
    end
