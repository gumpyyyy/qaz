...
describe StatusMessage do
  ...
  describe 'scopes' do
    describe '.where_person_is_mentioned' do
      it 'returns status messages where the given person is mentioned' do
        @bo = bob.person
        @test_string = "@{Daniel; #{@bo.lygneo_handle}} can mention people like Raph"
  ...
  describe '.after_create' do
    it 'calls create_mentions' do
      status = FactoryGirl.build(:status_message, text: "text @{Test; #{alice.lygneo_handle}}")
  ...
  describe '#lygneo_handle=' do
    it 'sets #author' do
      ...
      post.lygneo_handle = person.lygneo_handle
...
#    n.valid?.should be_false
    ...
    photo = @user.build_post(:photo, :user_file => uploaded_photo, :to => @group.id)
    photo.save!
  ...
  it 'should be postable through the user' do
    ...
    status = @user.post(:status_message, :text => message, :to => @group.id)
  ...
  describe 'mentions' do
    before do
      ...
      @test_string = <<-STR
@{Raphael; #{@people[0].lygneo_handle}} can mention people like Raphael @{Ilya; #{@people[1].lygneo_handle}}
can mention people like Raphaellike Raphael @{Daniel; #{@people[2].lygneo_handle}} can mention people like Raph
STR
    ...
    describe '#create_mentions' do
      it 'creates a mention for everyone mentioned in the message' do
        Lygneo::Mentionable.should_receive(:people_from_string).and_return(@people)
    ...
    describe "#filter_mentions" do
      it 'calls Lygneo::Mentionable#filter_for_groups' do
        msg = FactoryGirl.build(:status_message_in_group)

        msg_txt = msg.raw_message
        author_usr = msg.author.owner
        group_id = author_usr.groups.first.id

        Lygneo::Mentionable.should_receive(:filter_for_groups)
                             .with(msg_txt, author_usr, group_id)
      ...
      it "doesn't do anything when public" do
        ...
        Lygneo::Mentionable.should_not_receive(:filter_for_groups)
  ...
  describe "XML" do
    ...
    it 'serializes the author address' do
      @xml.should include(@user.person.lygneo_handle)
    end
    ...
    describe '.from_xml' do
      ...
      it 'marshals the lygneo_handle' do
        @marshalled.lygneo_handle.should == @message.lygneo_handle
      end
    end
  ...
  describe 'oembed' do
    before do
      ...
      @message_text = "#{@youtube_url} is so cool. so is this link -> https://joinlygneo.com"
    end
