...
describe Person do
  ...
  context 'scopes' do
    describe '.for_json' do
      ...
      it 'selects distinct people' do
        ...
        person_ids = Person.for_json.joins(:followers => :group_pledges).
          where(:followers => {:user_id => bob.id},
               :group_pledges => {:group_id => bob.group_ids}).map{|p| p.id}
    ...
    describe '.all_from_groups' do
      it "pulls back the right people given all a user's groups" do
        group_ids = bob.groups.map(&:id)
        Person.all_from_groups(group_ids, bob).map(&:id).should =~ bob.followers.includes(:person).map{|c| c.person.id}
      end

      it "pulls back the right people given a subset of groups" do
        group_ids = bob.groups.first.id
        Person.all_from_groups(group_ids, bob).map(&:id).should =~ bob.groups.first.followers.includes(:person).map{|c| c.person.id}
      end

      it "respects groups given a user" do
        group_ids = alice.groups.map(&:id)
        Person.all_from_groups(group_ids, bob).map(&:id).should == []
      end
    end
  ...
  describe '#lygneo_handle' do
    context 'local people' do
      it 'uses the pod config url to set the lygneo_handle' do
        new_person = User.build(:username => "foo123", :email => "foo123@example.com", :password => "password", :password_confirmation => "password").person
        new_person.lygneo_handle.should == "foo123#{User.lygneo_id_host}"
      end
      ...
      it 'does not include www if it is set in app config' do
        ...
        new_person = User.build(:username => "foo123", :email => "foo123@example.com", :password => "password", :password_confirmation => "password").person
        new_person.lygneo_handle.should == "foo123@foobar.com"
      end
    end

    context 'remote people' do
      it 'stores the lygneo_handle in the database' do
        @person.lygneo_handle.include?(AppConfig.pod_uri.host).should be false
      end
    end

    describe 'validation' do
      it 'is unique' do
        person_two = FactoryGirl.build(:person, :lygneo_handle => @person.lygneo_handle)
        person_two.should_not be_valid
      end

      it 'is case insensitive' do
        person_two = FactoryGirl.build(:person, :lygneo_handle => @person.lygneo_handle.upcase)
        person_two.should_not be_valid
      end
    end
  end
  ...
  context '.name_from_attrs' do
    ...
    context 'with only first name' do
      it 'should return their first name for name' do
        Person.name_from_attrs(@profile.first_name, nil, @profile.lygneo_handle).should == @profile.first_name.strip
      end
    end

    context 'with only last name' do
      it 'should return their last name for name' do
        Person.name_from_attrs(nil, @profile.last_name, @profile.lygneo_handle).should == @profile.last_name.strip
      end
    end

    context 'with both first and last name' do
      it 'should return their composed name for name' do
        Person.name_from_attrs(@profile.first_name, @profile.last_name, @profile.lygneo_handle).should == "#{@profile.first_name.strip} #{@profile.last_name.strip}"
      end
    end

    context 'without first nor last name' do
      it 'should display their lygneo handle' do
        Person.name_from_attrs(nil, nil, @profile.lygneo_handle).should == @profile.lygneo_handle
      end
    end
  end
  ...
  describe '#name' do
    it 'calls Person.name_from_attrs' do
      ...
      Person.should_receive(:name_from_attrs).with(profile.first_name, profile.last_name, profile.person.lygneo_handle)
      alice.name
    end
  ...
  describe "disconnecting" do
    before do
      ...
      @group = @user.groups.create(:name => "Dudes")
      @group2 = @user2.groups.create(:name => "Abscence of Babes")
    end
    it 'should not delete an orphaned follower' do
      @user.followers.create(:person => @person, :groups => [@group])

      lambda { @user.disconnect(@user.follower_for(@person)) }.should_not change(Person, :count)
    end

    it 'should not delete an un-orphaned follower' do
      @user.followers.create(:person => @person, :groups => [@group])
      @user2.followers.create(:person => @person, :groups => [@group2])
  ...
  describe '.search' do
    ...
    it 'returns results for Lygneo handles' do
      people = Person.search(@robert_grimm.lygneo_handle, @user)
      people.should == [@robert_grimm]
    end

    it "puts the searching user's followers first" do
      @robert_grimm.profile.first_name = "AAA"
      @robert_grimm.profile.save!
      ...
      @user.followers.create(:person => @casey_grippi, :groups => [@user.groups.first])
  ...
  context 'people finders for webfinger' do
    ...
    describe '.by_account_identifier' do
      it 'should find a local users person' do
        p = Person.by_account_identifier(user.lygneo_handle)
        p.should == user.person
      end

      it 'should find remote users person' do
        p = Person.by_account_identifier(person.lygneo_handle)
        p.should == person
      end

      it 'should downcase and strip the lygneo_handle' do
        dh_upper = "    " + user.lygneo_handle.upcase + "   "
      ...
      it "finds a local person with a mixed-case username" do
        ...
        person = Person.by_account_identifier(user.person.lygneo_handle)
        person.should == user.person
      end

      it "is case insensitive" do
        user1 = FactoryGirl.create(:user, :username => "SaMaNtHa")
        person = Person.by_account_identifier(user1.person.lygneo_handle.upcase)
        person.should == user1.person
      end

      it 'should only find people who are exact matches (1/2)' do
        user = FactoryGirl.create(:user, :username => "SaMaNtHa")
        person = FactoryGirl.create(:person, :lygneo_handle => "tomtom@tom.joinlygneo.com")
        user.person.lygneo_handle = "tom@tom.joinlygneo.com"
        user.person.save
        Person.by_account_identifier("tom@tom.joinlygneo.com").lygneo_handle.should == "tom@tom.joinlygneo.com"
      end

      it 'should only find people who are exact matches (2/2)' do
        person = FactoryGirl.create(:person, :lygneo_handle => "tomtom@tom.joinlygneo.com")
        person1 = FactoryGirl.create(:person, :lygneo_handle => "tom@tom.joinlygneo.comm")
        f = Person.by_account_identifier("tom@tom.joinlygneo.com")
        f.should be nil
      end
    ...
    describe '.local_by_account_identifier' do
      it 'should find local users people' do
        p = Person.local_by_account_identifier(user.lygneo_handle)
        p.should == user.person
      end

      it 'should not find a remote person' do
        p = Person.local_by_account_identifier(@person.lygneo_handle)
        p.should be nil
      end

      it 'should call .by_account_identifier' do
        Person.should_receive(:by_account_identifier)
        Person.local_by_account_identifier(@person.lygneo_handle)
      end
    end
  end
  describe '#has_photos?' do
    ...
    it 'returns true if the user has photos' do
      alice.post(:photo, :user_file => uploaded_photo, :to => alice.groups.first.id)
  ...
  describe '.popular' do
    describe "when the pod owner hasn't set up any community spotlight members" do
      it 'returns people with the community spotlight role' do
        ...
        Person.popular.should be_present
      end

      it "returns an empty array" do
        Person.popular.should == []
      end
    end
  end
