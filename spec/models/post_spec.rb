...
describe Post do
  before do
    @user = alice
    @group = @user.groups.create(:name => "winners")
  end
  ...
  describe 'scopes' do
    describe '.owned_or_visible_by_user' do
      before do
        ...
        @post_from_follower = eve.post(:status_message, :text => 'wooo', :to => eve.groups.where(:name => 'generic').first)
        @post_from_stranger = FactoryGirl.create(:status_message, :public => false)
      end

      it 'returns post from your followers' do
        StatusMessage.owned_or_visible_by_user(@you).should include(@post_from_follower)
      end
      ...
      it 'returns public post from your follower' do
      ...
      it 'does not return non followers, non-public post' do
    ...
    context 'having some posts' do
      before do
        ...
        time_past = 1000000
        @posts = (1..5).map do |n|
          group_to_post = alice.groups.where(:name => "generic").first
          post = alice.post :status_message, :text => "#{alice.username} - #{n}", :to => group_to_post.id
  ...
  describe 'serialization' do
    it 'should serialize the handle and not the sender' do
      post = @user.post :status_message, :text => "hello", :to => @group.id
      xml = post.to_lygneo_xml

      xml.include?("person_id").should be false
      xml.include?(@user.person.lygneo_handle).should be true
    end
  end

  describe '.lygneo_initialize' do
    it 'takes provider_display_name' do
      sm = FactoryGirl.create(:status_message, :provider_display_name => 'mobile')
      StatusMessage.lygneo_initialize(sm.attributes.merge(:author => bob.person)).provider_display_name.should == 'mobile'
    end
  end
  ...
  describe '#subscribers' do
    it 'returns the people contained in the groups the post appears in' do
      post = @user.post :status_message, :text => "hello", :to => @group.id

      post.subscribers(@user).should == []
    end

    it 'returns all a users followers if the post is public' do
      post = @user.post :status_message, :text => "hello", :to => @group.id, :public => true

      post.subscribers(@user).to_set.should == @user.follower_people.to_set
    end
  end
  ...
  describe "#receive_persisted" do
    before do
      @post = FactoryGirl.create(:status_message, :author => bob.person)
      @known_post = Post.new
      bob.stub(:follower_for).with(eve.person).and_return(stub(:receive_shareable => true))
    end
    ...
    context "the user does not know about the post" do
      ...
      it "receives the post from the follower of the author" do
        @post.send(:receive_persisted, bob, eve.person, @known_post).should == true
      end
  ...
  describe '#receive_non_persisted' do
    context "the user does not know about the post" do
      ...
      it "it receives the post from the follower of the author" do
        bob.should_receive(:follower_for).with(eve.person).and_return(stub(:receive_shareable => true))
  ...
  describe "#find_by_guid_or_id_with_user" do
    ...
    it "raises Lygneo::NonPublic for a non-existing id without a user" do
      Post.stub where: stub(includes: stub(first: nil))
      expect {
        Post.find_by_guid_or_id_with_user 123
      }.to raise_error Lygneo::NonPublic
    end

    it "raises Lygneo::NonPublic for a private post without a user" do
      post = FactoryGirl.create :status_message
      expect {
