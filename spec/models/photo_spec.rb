...
describe Photo do
  before do
    ...
    @photo  = @user.build_post(:photo, :user_file => File.open(@fixture_name), :to => @group.id)
    @photo2 = @user.build_post(:photo, :user_file => File.open(@fixture_name), :to => @group.id)
    @saved_photo = @user.build_post(:photo, :user_file => File.open(@fixture_name), :to => @group.id)
    @saved_photo.save
  end
  ...
  describe '#lygneo_initialize' do
    before do
      ...
      @photo = Photo.lygneo_initialize(
                :author => @user.person, :user_file => @image)
    end

    it 'sets the persons lygneo handle' do
      @photo.lygneo_handle.should == @user.person.lygneo_handle
    end
  ...
  describe 'serialization' do
    before do
      @saved_photo = with_carrierwave_processing do
         @user.build_post(:photo, :user_file => File.open(@fixture_name), :to => @group.id)
      end
    ...
    it 'serializes the lygneo_handle' do
      @xml.include?(@user.lygneo_handle).should be true
    end
  ...
  describe 'remote photos' do
    ...
    it 'should set the remote_photo on marshalling' do
      ...
      group2 = user2.groups.create(:name => "foobars")
      connect_users(@user, @group, user2, group2)

      url = @saved_photo.url
      thumb_url = @saved_photo.url :thumb_medium

      xml = @saved_photo.to_lygneo_xml
  ...
  context "deletion" do
    before do
      @status_message = @user.build_post(:status_message, :text => "", :to => @group.id)
