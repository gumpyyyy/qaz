...
describe AccountDeleter do
  before do
    @account_deletion = AccountDeleter.new(bob.person.lygneo_handle)
    @account_deletion.user = bob
  end

  it "attaches the user" do
    AccountDeleter.new(bob.person.lygneo_handle).user.should == bob
    AccountDeleter.new(remote_raphael.lygneo_handle).user.should == nil
  end
  ...
  describe '#perform' do
    ...
    person_removal_methods = [:delete_followers_of_me,
    ...
    context "person deletion" do
      before do
        @person_deletion = AccountDeleter.new(remote_raphael.lygneo_handle)
      end
  ...
  context 'person associations' do
    describe '#disconnect_followers' do
      it "deletes all of user's followers" do
        bob.followers.should_receive(:destroy_all)
        @account_deletion.disconnect_followers
      end
    end

    describe '#delete_followers_of_me' do
      it 'deletes all the local follower objects where deleted account is the person' do
        followers = mock
        Follower.should_receive(:all_followers_of_person).with(bob.person).and_return(followers)
        followers.should_receive(:destroy_all)
        @account_deletion.delete_followers_of_me
      end
    end
  ...
  describe "#remove_person_share_visibilities" do
    it 'removes the share visibilities for a person ' do
      @s_vis = stub
      ShareVisibility.should_receive(:for_followers_of_a_person).with(bob.person).and_return(@s_vis)
  ...
  describe "#remove_share_visibilities_by_followers_of_user" do
    it 'removes the share visibilities for a user' do
      @s_vis = stub
      ShareVisibility.should_receive(:for_a_users_followers).with(bob).and_return(@s_vis)
      @s_vis.should_receive(:destroy_all)

      @account_deletion.remove_share_visibilities_on_followers_posts
    end
  end
