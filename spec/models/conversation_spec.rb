  ...
  context 'transport' do
    ...
    describe 'serialization' do
      ...
      it 'serializes the participants' do
        @create_hash[:participant_ids].each{|id|
          @xml.should include(Person.find(id).lygneo_handle)
        }
      end
    ...
    describe '#subscribers' do
      it 'returns the recipients for the post owner' do
        @cnv.subscribers(@user1).should == @user1.followers.map{|c| c.person}
      end
    end
    ...
    describe '#receive' do
      ...
      it 'creates a message' do
        lambda{
          Lygneo::Parser.from_xml(@xml).receive(@user1, @user2.person)
        }.should change(Message, :count).by(1)
      end
      ...
      it 'does not save before receive' do
        Lygneo::Parser.from_xml(@xml).persisted?.should be_false
      end
