...
describe User do
  ...
  describe "validation" do
    ...
    describe "of username" do
      ...
      it 'requires uniqueness also amount Person objects with lygneo handle' do
        p = FactoryGirl.create(:person, :lygneo_handle => "jimmy#{User.lygneo_id_host}")
  ...
  describe '#update_profile' do
    ...
    it 'sends a profile to their followers' do
      mailman = Postzord::Dispatcher.build(alice, Profile.new)
      Postzord::Dispatcher.should_receive(:build).and_return(mailman)
      alice.update_profile(@params).should be_true
    end
    ...
    context 'passing in a photo' do
      before do
        ...
        @photo = Photo.lygneo_initialize(:author => alice.person, :user_file => image)
        @photo.save!
        @params = {:photo => @photo}
      end
  ...
  describe '#update_post' do
    it 'should dispatch post' do
      photo = alice.build_post(:photo, :user_file => uploaded_photo, :text => "hello", :to => alice.groups.first.id)
  ...
  describe '#notify_if_mentioned' do
    ...
    it 'does not notify the user if the post author is not a follower' do
  ...
  describe 'account deletion' do
    describe '#destroy' do
      ...
      it 'removes invitations to the user' do
        Invitation.new(:sender => eve, :recipient => alice, :identifier => alice.email, :group => eve.groups.first).save(:validate => false)
        lambda {
          alice.destroy
  ...
  describe '#mail' do
    it 'enqueues a mail job' do
      ...
      Workers::Mail::StartedSharing.should_receive(:perform_async).with(alice.id, 'followerrequestid').once
      alice.mail(Workers::Mail::StartedSharing, alice.id, 'followerrequestid')
    end
  ...
  context "group management" do
    before do
      ...
      @original_group = alice.groups.where(:name => "generic").first
      @new_group = alice.groups.create(:name => 'two')
    end

    describe "#add_follower_to_group" do
      it 'adds the follower to the group' do
        lambda {
          alice.add_follower_to_group(@follower, @new_group)
        }.should change(@new_group.followers, :count).by(1)
      end

      it 'returns true if they are already in the group' do
        alice.add_follower_to_group(@follower, @original_group).should be_true
      end
    end
  end
  ...
  context 'likes' do
    before do
      ...
      @message = alice.post(:status_message, :text => "cool", :to => alices_group)
      @message2 = bob.post(:status_message, :text => "uncool", :to => @bobs_group)
  ...
  describe "#seed_groups" do
    describe "create groups" do
      ...
      [I18n.t('groups.seed.family'), I18n.t('groups.seed.friends'),
       I18n.t('groups.seed.work'), I18n.t('groups.seed.acquaintances')].each do |group_name|
        it "creates an group named #{group_name} for the user" do
          user.groups.find_by_name(group_name).should_not be_nil
        end
      end
    end
    ...
    describe "autofollow sharing" do
      ...
      context "with sharing with lygneohq enabled" do
        it "should not start sharing with the lygneohq account" do
