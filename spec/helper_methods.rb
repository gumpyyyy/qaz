module HelperMethods
  def connect_users_with_groups(u1, u2)
    group1 = u1.groups.length == 1 ? u1.groups.first : u1.groups.where(:name => "Besties").first
    group2 = u2.groups.length == 1 ? u2.groups.first : u2.groups.where(:name => "Besties").first
    connect_users(u1, group1, u2, group2)
  end
  def connect_users(user1, group1, user2, group2)
    user1.followers.create!(:person => user2.person,
                           :groups => [group1],
                           :sharing => true,
                           :receiving => true)

    user2.followers.create!(:person => user1.person,
                           :groups => [group2],
                           :sharing => true,
                           :receiving => true)
  end
  ...
  def stub_success(address = 'abc@example.com', opts = {})
    ...
    if opts[:lygneo] || host.include?("lygneo")
