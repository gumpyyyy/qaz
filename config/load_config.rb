...
AppConfig ||= Configurate::Settings.create do
  ...
  unless heroku? || rails_env == "test" || File.exists?(File.join(config_dir, 'lygneo.yml'))
    $stderr.puts "FATAL: Configuration not found. Copy over lygneo.yml.example"
    $stderr.puts "       to lygneo.yml and edit it to your needs."
    exit!
  end
  
  add_provider Configurate::Provider::YAML,
               File.join(config_dir, 'lygneo.yml'),
  ...
  if rails_env == "production"  &&
    ...
    $stderr.puts "FATAL: Lygneo doesn't know where your certificate authorities are. Please ensure they are set to a valid path in lygneo.yml"
    exit!
  end
end
