...
When 'I click the groups title' do
  find('.home_selector').click
end

When /^I press the group dropdown$/ do
  find('.dropdown .button').click
end

And /^I toggle the group "([^"]*)"$/ do |group_name|
  group = @me.groups.where(:name => group_name).first
  find(".dropdown li[data-group_id='#{group.id}']").click
end
...
Then /^I should see (\d+) followers$/ do |n_posts|
