...
Given /^"([^"]*)" has a public post with text "([^"]*)"$/ do |email, text|
  ...
  user.post(:status_message, :text => text, :public => true, :to => user.groups)
end

Given /^"([^"]*)" has a non public post with text "([^"]*)"$/ do |email, text|
  user = User.find_by_email(email)
  user.post(:status_message, :text => text, :public => false, :to => user.groups)
end

And /^the post with text "([^"]*)" is reshared by "([^"]*)"$/ do |text, email|
  user = User.find_by_email(email)
  root = Post.find_by_text(text)
  user.post(:reshare, :root_guid => root.guid, :public => true, :to => user.groups)
end
...
When /^I select "([^"]*)" on the group dropdown$/ do |text|
  page.execute_script(
