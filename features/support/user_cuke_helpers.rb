module UserCukeHelpers
  ...
  # and the given override attributes, adds the standard groups to it
  # and returns it
  def create_user(overrides={})
    ...
    add_standard_groups(user)
    user
  end

  # create the default testing groups for a given user
  def add_standard_groups(user)
    user.groups.create(:name => "Besties")
    user.groups.create(:name => "Unicorns")
  end
  ...
  def automatic_login
    @me ||= FactoryGirl.create(:user_with_group, :getting_started => false)
